...
let accordion = document |> Document.querySelector(".accordion") |> unwrapElement;
...
let composeItem = (node) => {
  ...
  let intialHeight =
    section |> getClassList |> DomTokenList.contains("collapsed") ? headerHeight : contentHeight;
  section |> Element.setAttribute("style", "max-height:" ++ string_of_int(intialHeight) ++ "px");
  header |> Element.addClickEventListener((e) => handleHeaderClick(e, headerHeight, contentHeight));
  {
    header,
    content,
